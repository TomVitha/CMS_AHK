#Hotstring EndChars             ; * No ending character needs to be typed after the abbreviation to trigger
...
:*b0:<address>::</address>{left 10}
; :*b0:<area>::{left 0}
:*b0:<article>::</article>{left 10}
...
:*b0:<map>::</map>{left 6}
:*b0:<mark>::</mark>{left 7}
; :*b0:<marquee>::</marquee>{left 10}
:*b0:<menu>::</menu>{left 7}
; :*b0:<meta>::{left 0}
:*b0:<meter>::</meter>{left 8}
:*b0:<nav>::</nav>{left 6}
; :*b0:<nobr>::</nobr>{left 7}
; :*b0:<noembed>::</noembed>{left 10}
...
:*b0:<p>::</p>{left 4}
:*b0:<param>::{left 0}
:*b0:<picture>::</picture>{left 10}
; :*b0:<plaintext>::</plaintext>{left 12}
...
:*b0:<sub>::</sub>{left 6}
:*b0:<summary>::</summary>{left 10}
:*b0:<sup>::</sup>{left 6}
:*b0:<table>::</table>{left 8}
:*b0:<tbody>::</tbody>{left 8}
:*b0:<td>::</td>{left 5}
:*b0:<template>::</template>{left 11}
:*b0:<textarea>::</textarea>{left 11}
:*b0:<tfoot>::</tfoot>{left 8}
:*b0:<th>::</th>{left 5}
:*b0:<thead>::</thead>{left 8}
:*b0:<time>::</time>{left 7}
:*b0:<title>::</title>{left 8}
:*b0:<tr>::</tr>{left 5}
:*b0:<track>::</track>{left 8}
; :*b0:<tt>::</tt>{left 5}
; :*b0:<u>::</u>{left 4}
:*b0:<ul>::</ul>{left 5}
:*b0:<var>::</var>{left 6}
; :*b0:<video>::</video>{left 8}
; :*b0:<wbr>::{left 0}
; :*b0:<xmp>::</xmp>{left 6}